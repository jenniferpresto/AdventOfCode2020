...
class Day17
{
    ...
    private void doPartTwo()
    {
        ...
        int initialDimension = data[0].Length;
        //  create two arrays of identical dimensions
        int[,,,] dataArray = new int[initialDimension + (numCycles * 2), initialDimension + (numCycles * 2), initialDimension + (numCycles * 2), initialDimension + (numCycles * 2)];
        int[,,,] calculatedDataArray = new int[initialDimension + (numCycles * 2), initialDimension + (numCycles * 2), initialDimension + (numCycles * 2), initialDimension + (numCycles * 2)];
    ...
    private void initializeArrayToZeroes(int[,,,] dataArray)
    {
        for (int x = 0; x < dataArray.GetLength(0); x++)
        {
            for (int y = 0; y < dataArray.GetLength(1); y++)
            {
                for (int z = 0; z < dataArray.GetLength(2); z++)
                {
                    for (int w = 0; w < dataArray.GetLength(3); w++)
                    {
                        dataArray[x, y, z, w] = 0;
                    }
                }
        ...
        for (int x = 0; x < dataArray.GetLength(0); x++)
        {
            for (int y = 0; y < dataArray.GetLength(1); y++)
            {
                for (int z = 0; z < dataArray.GetLength(2); z++)
                {
                    for (int w = 0; w < dataArray.GetLength(3); w++)
                    {
                        if (testSpecificCoordinate(x, y, z, w, dataArray))
                        {
                            calculatedDataArray[x, y, z, w] = 1;
                        }
                        else
                        {
                            calculatedDataArray[x, y, z, w] = 0;
                        }
                    }
                }
            }
    ...
    private bool testSpecificCoordinate(int x, int y, int z, int w, int[,,,] dataArray)
    {
        bool active = false;
        int totalActiveNeighbors = 0;
        for (int i = x - 1; i < x + 2; i++)
        {
            ...
            for (int j = y - 1; j < y + 2; j++)
            {
                ...
                for (int k = z - 1; k < z + 2; k++)
                {
                    ...
                    {
                        if (m < 0 || m > dataArray.GetLength(3) - 1) continue;
                        if (dataArray[i, j, k, m] == 1)
                        {
                            if (x == i && y == j && z == k && w == m) { continue; }
                            totalActiveNeighbors++;
                        }
                    }
                }
            ...
            for (int y = 0; y < sideOfCube; y++)
            {
                for (int x = 0; x < sideOfCube; x++)
                {
                    for (int w = 0; w < sideOfCube; w++)
                    {
                        ...
                        {
                            newVal = calculatedArray[x - 1, y - 1, z - 1, w - 1];
                        }
                        originalArray[x, y, z, w] = newVal;
                    }
                }
            }
        }
        ...
        for (int z = 0; z < dataArray.GetLength(2); z++)
        {
            for (int w = 0; w < dataArray.GetLength(3); w++)
            {
                ...
                for (int y = 0; y < dataArray.GetLength(1); y++)
                {
                    for (int x = 0; x < dataArray.GetLength(0); x++)
                    {
                        Console.Write(dataArray[x, y, z, w]);
                    }
                    Console.WriteLine();
                }
